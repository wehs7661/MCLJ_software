...
def adjust_displacement(n_trials, n_accept, max_displacement):
    ...
    n_trials = 0
    n_accept = 0
    return n_trials, n_accept, max_displacement
...
if __name__ == "__main__":
    ...
    coordinates = generate_initial_coordinates(
    ...
    for i_step in range(n_steps):
        ...
        current_energy = get_particle_energy(
            coordinates, i_particle, simulation_cutoff2)
        ...
        proposed_energy = get_particle_energy(
            proposed_coordinates, i_particle, simulation_cutoff2)
        ...
        if np.mod(i_step + 1, freq) == 0:
            ...
            if tune_displacement:
                [n_trials, n_accept, max_displacement] = adjust_displacement(
    ...
    delta_t = end - start
    print(
        f'Average total energy of particles of last 1% of steps: {np.mean(energy_array[-int(0.1 * len(energy_array)):])}')
    print(f'Time elasped: {delta_t} seconds')
    print(
        f'simulation speed: {delta_t / n_steps * 1000} seconds per 1000 steps')
    ...
    plt.plot(range(5001, 5001 + len(energy_array[5000:])), energy_array[5000:])
    if 5001 + len(energy_array[5000:]) >= 10000:
        plt.ticklabel_format(style='sci', axis='x', scilimits=(0, 0))
    plt.xlabel('Monte Carlo steps')
    plt.ylabel('Energy (reduced units)')
    plt.title('Particle energy as a function of Monte Carlo steps')
    plt.grid(True)
    ...
    plt.title('The final configurations in 3D space')
    plt.savefig('final_configuration.png')
    plt.show()
